...
class Game:
    def __init__(self, height, width):
        ...
        self.objects = []
        self.maps = [self.createmap1(), self.createmap2(), self.createmap3()]
        self.current_map = 0
    ...
    def CreateDoorObject(self, name: str, position: np.ndarray, radius: float=40.0):
        ...
        verts, inds = CreateStone(radius=radius, color=[0.9, 0.9, 0.85])
    ...
    def createmap3(self):
        ...
        player.properties['position'] = np.array([-450, -450, 0], dtype=np.float32)
        objs =  [space, player]
        stone_objs = []

        entry_door = self.CreateDoorObject('entry_door', np.array([-450, -450, 0], dtype=np.float32), radius=35)
        ...
        for i in range(8):
            ...
            stone_verts, stone_inds = CreateStone(radius=r, color=[0.96, 0.87, 0.70])  # Changed to brownish color
    ...
    def createmap2(self):
        ...
        for i in range(8):
            ...
            stone_verts, stone_inds = CreateStone(radius=r, color=[0.5, 0.3, 0.1])  # Changed to brownish color
            stone_obj = Object(self.shaders[0], {
                'name': 'stone',
                'vertices': np.array(stone_verts, dtype=np.float32),
                'indices': np.array(stone_inds, dtype=np.uint32),
    ...
    def UpdateScene(self, inputs, time):
        ...
        if self.screen == 1:
            ...
            if jump_key in inputs:
                self.jump_charge_time += delta
                self.jump_charge_time = min(self.jump_charge_time, 7.0)  
            else:
                if self.jump_charge_time > 0.0:
                    distance_factor = self.jump_charge_time * 200.0
                    jump_dist = max(min_jump, min(max_jump, distance_factor))
            ...
            if not hasattr(self, 'time_off_rock'):
                self.time_off_rock = 0.0
            ...
            if self.player_on_rock is not None and player_obj:
                ...
                x, y, z = player_obj.properties['position']
                if -365.0 <= x <= 365.0:
                    self.time_off_rock += delta

                if self.time_off_rock > 0.4:
                    self.health -= 10 
                    self.time_off_rock += delta
                    self.time_off_rock = 0.0
                    ...
                    if self.health <= 0:
                        ...
                        self.time_off_rock = 0.0
                        safe_respawn_position = np.array([-450, -450.0, 0.0], dtype=np.float32)
                        player_obj.properties['position'] = safe_respawn_position
